    ...
    <div class="grid grid-4" style="margin-bottom: 24px;">
      ...
      <div class="stat-card muted">
        ...
        <div class="stat-value" style="font-size: 1.5rem;">
          {{ recentChecks.length ? formatDiffForHumans(recentChecks[0].checked_at) : 'Never' }}
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-3" style="gap: 24px; margin-bottom: 24px;">
      ...
      <div class="card" style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px;text-align:center;">
        ...
        <div style="font-family:var(--font-display);font-size:1rem;font-weight:700;color:var(--color-text)">
          <span v-if="overallScore >= 80">All Systems Nominal</span>
          <span v-else-if="overallScore >= 60">Attention Required</span>
          <span v-else>Active Threats Detected</span>
        </div>
        <div style="font-size:0.8rem;color:var(--color-muted);margin-top:6px">
          {{ projects.length }} projects monitored
        </div>
        <div v-if="p1Count > 0" class="badge danger" style="margin-top:12px;font-size:0.78rem;padding:6px 14px;border-radius:999px;animation:blink 1.2s ease-in-out infinite">
      ...
      <div class="card" style="grid-column: span 2">
        ...
        <div class="card-body" style="padding:0">
          <template v-if="openIncidents.length > 0">
            <a v-for="incident in openIncidents" :key="incident.id" :href="'/incidents/' + incident.id" style="text-decoration:none;color:inherit;display:block;">
              <div style="display:flex;align-items:center;gap:14px;padding:14px 20px;border-bottom:1px solid var(--color-border);transition:background .15s;" class="hover-surface">
                <span class="badge" :class="getBadgeClass(incident.severity)">{{ getSeverityLabel(incident.severity) }}</span>
                <div style="flex:1;min-width:0">
                  ...
                  <div style="font-size:0.75rem;color:var(--color-muted)">{{ incident.project.domain }} · {{ formatDiffForHumans(incident.detected_at) }}</div>
                </div>
                <span class="badge muted">{{ incident.status.replace('_', ' ') }}</span>
                <span v-if="incident.sla_respond_breached" class="sla-timer breach">SLA BREACH</span>
              </div>
            </a>
          </template>
          <div v-else style="padding:40px;text-align:center;color:var(--color-success)">
          ...
          <template v-if="projects.length > 0">
            <a v-for="project in projects.slice(0, 8)" :key="project.id" :href="'/projects/' + project.id" style="text-decoration:none;color:inherit;display:block;">
              <div style="display:flex;align-items:center;gap:12px;padding:12px 20px;border-bottom:1px solid var(--color-border);transition:background .15s" class="hover-surface">
                ...
                <span v-if="project.incidents.length > 0" class="badge danger">{{ project.incidents.length }} incidents</span>
          ...
          <table class="table" style="width:100%; border-collapse:collapse; text-align:left;">
            ...
            <tbody>
              <tr v-for="check in recentChecks" :key="check.id" style="border-bottom:1px solid #334155;">
                <td style="padding:12px 20px;"><span style="font-family:var(--font-mono); font-size:0.82rem;">{{ check.project.domain }}</span></td>
                ...
                <td style="padding:12px 20px;">{{ check.log_lines_scanned.toLocaleString() }}</td>
                <td style="padding:12px 20px;"><span :style="{ color: check.errors_found > 0 ? '#ef4444' : '#94a3b8' }">{{ check.errors_found }}</span></td>
                <td style="padding:12px 20px;"><span :style="{ color: check.warnings_found > 0 ? '#f59e0b' : '#94a3b8' }">{{ check.warnings_found }}</span></td>
              </tr>
            </tbody>
          </table>
        </template>
...
onMounted(() => {
  if (typeof ApexCharts !== 'undefined' && chartEl.value) {
    new ApexCharts(chartEl.value, {
      ...
      series: [{ name: 'Errors', data: props.chartData.series }],
      xaxis: {
